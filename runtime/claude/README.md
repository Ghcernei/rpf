...
- **Reader detail level 3 = reasoning, not chronicle (INFO-016).** Five mandatory beats: role choice justified from the nature of the question; each role's presumption (built-in view, loss function); each subagent's mandate and input; the parent's weighing with conflict classification (fact → evidence and resolution; value → E8 gate); the decision trace (rule/precedent applied, what went to `decisions/`). Anti-rationalization: level 3 renders the already-mandatory O5.1 / O9.1 / SY-classification records in human language and writes no new justifications — missing render material is reported as a discipline-violation finding, never invented post-hoc. The render itself runs at tier S; total narrative budget stays ≤5% of subtree cost. Commercially, level 3 is the Flow Support premium mode and the explainability artifact of the banking vertical.
