...
# ---------------------------------------------------------------------------
# Scheduler abstraction (ATOM-150, H1): ONE detector + ONE dispatcher over
# three backends — launchd (macOS), systemd user timers (Linux/WSL2 with
# systemd), cron (fallback). Every scheduling consumer (the Telegram
# listener + digest, the morning heartbeat) enables through _sched_run.
# The launchd backend keeps the EXACT pre-ATOM-150 path: its bootstrap
# functions (_telegram_bootstrap_attempt / _heartbeat_enable_attempt) are
# called verbatim and the plists are generated by the same unchanged code,
# so a Mac install does not change one byte — the harness proves it with cmp
# on the generated plists and the launchctl call log. No new interview
# question: the backend is auto-detected. When nothing is found the caller
# prints an honest L_*_NO_SCHEDULER message pointing at docs/WINDOWS.md —
# never a silent, never a half-install (INPUT §2 H1, S1 / INFO-044).
# ---------------------------------------------------------------------------
SCHED_BACKEND=""   # last detection result (launchd|systemd|cron|none) — persisted
SCHED_USED=""      # backend the last _sched_run actually dispatched to

_sched_backend() {
  # launchd FIRST so macOS is always the unchanged path; QROKY_SCHED_BACKEND
  # lets the harness force a backend deterministically. systemd only counts
  # when it is truly the running init AND the per-user bus answers (a WSL2
  # box without "systemd=true" has systemctl on PATH but no /run/systemd).
  if [[ -n "${QROKY_SCHED_BACKEND:-}" ]]; then SCHED_BACKEND="$QROKY_SCHED_BACKEND"
  elif command -v launchctl >/dev/null 2>&1; then SCHED_BACKEND="launchd"
  elif command -v systemctl >/dev/null 2>&1 && [[ -d /run/systemd/system ]] \
       && systemctl --user show-environment >/dev/null 2>&1; then SCHED_BACKEND="systemd"
  elif command -v crontab >/dev/null 2>&1; then SCHED_BACKEND="cron"
  else SCHED_BACKEND="none"; fi
  printf '%s' "$SCHED_BACKEND"
}

# Dispatch one consumer's enable across the detected backend, keeping the
# launchd attempt function CALLED VERBATIM (byte parity). Sets SCHED_USED.
# Returns 0 scheduled, 1 attempt failed, 3 no scheduler present at all.
#   $1 ladder tag   $2 launchd fn   $3 systemd fn   $4 cron fn
_sched_run() {
  ...
  case "$SCHED_USED" in
    ...
    cron)    run_with_ladder "$tag" "$cron_fn"; return $? ;;
    *)       return 3 ;;
  esac
}

# Rewrite this user's crontab so exactly one qroky block (bounded by the
# marker pair) carries the given body. Idempotent: a previous block for the
# same markers is dropped first, every other crontab line is preserved.
#   $1 begin marker   $2 end marker   $3.. body lines (each a full cron line)
_crontab_replace_block() {
  local begin="$1" end="$2"; shift 2
  local current stripped
  current="$(crontab -l 2>/dev/null || true)"
  stripped="$(printf '%s\n' "$current" | awk -v b="$begin" -v e="$end" '
    $0==b {skip=1; next} $0==e {skip=0; next} skip!=1 {print}')"
  {
    printf '%s\n' "$stripped" | grep -v '^[[:space:]]*$' || true
    printf '%s\n' "$begin"
    printf '%s\n' "$@"
    printf '%s\n' "$end"
  } | crontab - 2>>"${LOG_FILE:-/dev/null}"
}

# --- systemd user-timer backend (Linux/WSL2) --------------------------------
_telegram_systemd_attempt() {
  local ud="$HOME/.config/systemd/user"
  mkdir -p "$ud" || return 1
  cat > "$ud/qroky-telegram-listener.service" <<EOF
[Unit]
Description=Qroky Telegram listener (one pass)
[Service]
Type=oneshot
ExecStart=/bin/bash $TG_HOME_DIR/run-listener.sh
EOF
  cat > "$ud/qroky-telegram-listener.timer" <<EOF
[Unit]
Description=Qroky Telegram listener every 30 seconds
[Timer]
OnBootSec=30
OnUnitActiveSec=30
AccuracySec=1s
...
EOF
  cat > "$ud/qroky-telegram-digest.service" <<EOF
[Unit]
Description=Qroky Telegram daily digest
[Service]
Type=oneshot
ExecStart=/bin/bash $TG_HOME_DIR/run-digest.sh
EOF
  cat > "$ud/qroky-telegram-digest.timer" <<EOF
[Unit]
Description=Qroky Telegram daily digest at 09:05
[Timer]
OnCalendar=*-*-* 09:05:00
Persistent=true
[Install]
WantedBy=timers.target
EOF
  systemctl --user daemon-reload 2>>"${LOG_FILE:-/dev/null}" || return 1
  systemctl --user enable --now qroky-telegram-listener.timer qroky-telegram-digest.timer \
    2>>"${LOG_FILE:-/dev/null}" || return 1
  return 0
}

_heartbeat_systemd_attempt() {
  local ud="$HOME/.config/systemd/user" base unit
  base="$(basename "$WORKSPACE_DIR")"
  unit="qroky-heartbeat-$base"
  mkdir -p "$ud" || return 1
  cat > "$ud/$unit.service" <<EOF
[Unit]
Description=Qroky morning digest ($base)
[Service]
Type=oneshot
ExecStart=/bin/bash $WORKSPACE_DIR/.qroky/heartbeat.sh
EOF
  cat > "$ud/$unit.timer" <<EOF
[Unit]
Description=Qroky morning digest on weekdays at 09:07 ($base)
[Timer]
OnCalendar=Mon..Fri *-*-* 09:07:00
Persistent=true
[Install]
WantedBy=timers.target
EOF
  systemctl --user daemon-reload 2>>"${LOG_FILE:-/dev/null}" || return 1
  systemctl --user enable --now "$unit.timer" 2>>"${LOG_FILE:-/dev/null}" || return 1
  return 0
}

# --- cron fallback backend --------------------------------------------------
# cron cannot fire sub-minute, so the listener runs every minute here (an
# honest degradation from launchd/systemd's 30 s — the digest and heartbeat
# keep their exact times). The WSL2 cron-service caveat is surfaced by the
# caller via L_SCHED_CRON_NOTE.
_telegram_cron_attempt() {
  _crontab_replace_block \
    "# >>> qroky-telegram (managed by the Qroky installer) >>>" \
    "# <<< qroky-telegram <<<" \
    "* * * * * /bin/bash $TG_HOME_DIR/run-listener.sh >/dev/null 2>&1" \
    "5 9 * * * /bin/bash $TG_HOME_DIR/run-digest.sh >/dev/null 2>&1"
}

_heartbeat_cron_attempt() {
  local base; base="$(basename "$WORKSPACE_DIR")"
  _crontab_replace_block \
    "# >>> qroky-heartbeat-$base (managed by the Qroky installer) >>>" \
    "# <<< qroky-heartbeat-$base <<<" \
    "7 9 * * 1-5 /bin/bash $WORKSPACE_DIR/.qroky/heartbeat.sh >/dev/null 2>&1"
}
...
state_load() {
  ...
  SCHED_BACKEND="$(state_get sched_backend || true)"
  return 0
}
...
state_commit() {
  ...
  mkdir -p "$(dirname "$STATE_FILE")"
  {
    ...
    printf '  "sched_backend": "%s",\n' "$(json_escape "${SCHED_BACKEND:-}")"
...
EOF
  ...
  if _sched_run telegram_jobs _telegram_bootstrap_attempt _telegram_systemd_attempt _telegram_cron_attempt; then
    L_TELEGRAM_DEPLOYED
    if [[ "$SCHED_USED" == "cron" ]]; then L_SCHED_CRON_NOTE; fi
    log "telegram HEAD-DEPLOYED via $SCHED_USED (listener 30s; digest 09:05 daily; state=$TG_HOME_DIR)"
  elif [[ "$SCHED_USED" == "none" ]]; then
    L_TELEGRAM_NO_SCHEDULER "$TG_HOME_DIR"
    log "telegram NO-SCHEDULER (files in place; manual scheduling instruction shown)"
  else
    L_TELEGRAM_SCHEDULE_FAILED
    log "telegram JOBS-ENABLE-FAILED-TO-HUMAN via $SCHED_USED (files in place; enable-later shown)"
  fi
...
step_heartbeat() {
  ...
  if is_affirmative "$ans" || [[ -z "$ans" ]]; then
    if _sched_run heartbeat _heartbeat_enable_attempt _heartbeat_systemd_attempt _heartbeat_cron_attempt; then
      L_HEARTBEAT_ON
      if [[ "$SCHED_USED" == "cron" ]]; then L_SCHED_CRON_NOTE; fi
      ANSWER_HEARTBEAT_OPTIN="yes"
    elif [[ "$SCHED_USED" == "none" ]]; then
      L_HEARTBEAT_NO_SCHEDULER "$WORKSPACE_DIR"
      ANSWER_HEARTBEAT_OPTIN="no"
    else
      L_HEARTBEAT_SCHEDULE_FAILED
      ANSWER_HEARTBEAT_OPTIN="no"
      log "heartbeat ENABLE-FAILED-TO-HUMAN via $SCHED_USED (installed disabled; enable instruction shown)"
    fi
  else
    L_HEARTBEAT_OFF
    ANSWER_HEARTBEAT_OPTIN="no"
...
cmd_enable_heartbeat() {
  ...
  if _sched_run heartbeat _heartbeat_enable_attempt _heartbeat_systemd_attempt _heartbeat_cron_attempt; then
    ...
    L_HEARTBEAT_SCHEDULE_FAILED
    log "heartbeat ENABLE-FAILED-TO-HUMAN (--enable-heartbeat via $SCHED_USED)"
    exit 1
  fi
}
...
cmd_uninstall() {
  ...
  # 1b) systemd user timers (Linux/WSL2): disable + remove any qroky units.
  # Skipped on a machine without systemctl (macOS) — same launchd behaviour
  # as before, not a byte different there.
  if command -v systemctl >/dev/null 2>&1; then
    local ud="$HOME/.config/systemd/user" ufile uname
    if [[ -d "$ud" ]]; then
      for ufile in "$ud"/qroky-telegram-listener.timer "$ud"/qroky-telegram-digest.timer \
                   "$ud"/qroky-heartbeat-*.timer; do
        [[ -f "$ufile" ]] || continue
        uname="$(basename "$ufile")"
        L_UNINSTALL_STEP "systemd: $uname"
        systemctl --user disable --now "$uname" 2>/dev/null || true
        _un_note "systemd: $uname"
      done
  ...
  # 1c) cron fallback lines: strip every qroky-managed crontab block. Reads
  # the crontab read-only and rewrites ONLY when a qroky marker is actually
  # present — a Mac (where cron was never our backend) keeps its crontab
  # untouched.
  if command -v crontab >/dev/null 2>&1; then
    local cronnow
    cronnow="$(crontab -l 2>/dev/null || true)"
    if printf '%s\n' "$cronnow" | grep -qE 'qroky-telegram|qroky-heartbeat'; then
      L_UNINSTALL_STEP "cron: qroky jobs"
      printf '%s\n' "$cronnow" | awk '
